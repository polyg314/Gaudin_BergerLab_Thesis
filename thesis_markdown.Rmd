...
```{r}
st_alt_table <- targeted_normal_read_table[[1]][,3:ncol(targeted_normal_read_table[[1]]) - 1] ## remove chromosome and position
st_cov_table <- targeted_normal_read_table[[2]][,3:ncol(targeted_normal_read_table[[2]]) - 1] ## remove chromosome and position
avg_alts <- c() 
avg_covs <- c()
pool_vector <- c()
sds <- c()
...
for(i in 1:nrow(st_alt_table)){
  ...
  if(length(indices) > 7){ ##only average SNPs that are heterozygous at at least 7 SNP sites
    ...
    size <- c(size, length(indices))
  }
}
cov_df <- data.frame(Average_vaf = avg_alts, Average_coverage = avg_covs, pool = pool_vector,Standard_devs = sds, size = size)
cov_df <- cov_df[!is.na(cov_df$Average_vaf),]
print("number of SNPs included")
print(nrow(cov_df))## print number of SNPs 
print(ggplot(cov_df, aes(x=Average_coverage, y=Average_vaf, color = pool)) + xlab("SNP Site Average Coverage") + ylab("Mean VAF for SNP") + geom_point() + geom_vline(aes(xintercept=250), colour="#BB0000", linetype="dashed") + ylim(5,95))
print(ggplot(cov_df, aes(x=Average_coverage, y=Standard_devs, shape = pool, color = size )) + xlab("SNP Site Average Coverage") + ylab("Standard Dev for het SNP") + geom_point() + geom_vline(aes(xintercept=250), colour="#BB0000", linetype="dashed"))
...
print(nrow(cov_df))## print number of SNPs 
print(ggplot(cov_df, aes(x=Average_coverage, y=Average_vaf, color = pool)) + xlab("SNP Site Average Coverage") + ylab("Mean VAF for SNP") + geom_point() + geom_vline(aes(xintercept=250), colour="#BB0000", linetype="dashed") + ylim(5,95))
print(ggplot(cov_df, aes(x=Average_coverage, y=Standard_devs, color = pool)) + xlab("SNP Site Average Coverage") + ylab("Standard Dev for het SNP") + geom_point() + geom_vline(aes(xintercept=250), colour="#BB0000", linetype="dashed"))
```
...
density_df
p <- ggplot(density_df, aes(x=VAF_Z_Score, color = samples)) + geom_density() + xlim(0.5,2)
p
```
...
density_df <- rbind(density_normal, density_all, density_0, density_above_0, density_above_0.05)
density_df
p <- ggplot(density_df, aes(x=chrom_triple_max, color = samples)) + geom_density() + xlim(0.7,2)
p
```
...
density_df <- rbind(density_normal, density_all, density_0, density_above_0, density_above_0.05)
density_df
p <- ggplot(density_df, aes(x=abs_frag_z_score, color = samples)) + geom_density() + xlim(0,4)
p
...
#### ROC curves for classifying samples as clinical cancer patient samples or normal samples (n = 47) using both VAF Z-score and fragment length Z-score
```{r}
print("roc_df_0")
...
###### TSNE
```{r}
tsne_table_sd_scaled <- frag_table[c(normal_indices,tumor_all_indices),fragment_bins_to_analyze]
